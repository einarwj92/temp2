...
import os
from util import IMG_file, IMAG_file, DSC_file, get_list_of_types
import sys
import time
import shutil
import exifread
...
def remove_postfix(filename):
    ...
    if "IMG" in filename:
        ...
        # metode for masse postfix og div.
        name, ext = os.path.splitext(filename)
        nn = name[:19] + ext
        #print(nn)
        return nn
        ...
        """
        brukt på 
        IMG_20210114_153946(123456).jpg
        IMG_20210114_153946(12345).jpg
...
src = "E:\\src\\bilder"
dst = "E:\\full unik sortering 030323" # root
...
remove_postfix_first = False
include_IMG = True     # ok    2 fantes fra før
...
if main:
    ...
    for dirpath, dirs, filenames in os.walk(src):
        ...
        for filename in filenames:
            ...
            # remove postfix
            filename2 = filename
            if include_IMG and remove_postfix_first:
                if "IMG" in filename:
                    filename2 = remove_postfix(filename)
                else:
                    continue
            """
                # brukt på de fleste av duplikat IMG filer (123456) (12345)
                if ("(" in filename) and (29 < len(filename) < 32):
            ...
            # exclude files
            # give img tag
            img_tag = accepted_file(filename2, accepted_formats)    # om remove av postfix av success, vil denne metode avsløre v å ret 'mg'
            if not img_tag:
            ...
            if not os.path.isfile(newfile):
                ...
                print("Not in dst, moving: ",filename, new_filename)
            else:
                #print("Already in dst: ",filename, new_filename)
                counter_already_in_dst += 1
                counter_accepted -= 1
                if move:
                    newfile2 = os.path.join("D:\\kan slettes\\rmpostfixbutindstandwrdformat\\a", new_filename)
                    if not os.path.isfile(newfile2):
                        shutil.move(srcfile, newfile2)
